...
class PedidoBloc {
  ...
  Stream<List<Categoria>> get getCategoriasProdutos => _categoriaProdutosSC.stream;
  ...
  void fetch(BuildContext context){
    ...
    DefaultAssetBundle.of(context)
        ...
        .then((clientesString) {
      final data = json.decode(clientesString);
      _listCategorias = data.map<Categoria>((json) => Categoria.fromJson(json))
          .toList();
  ...
  void setOptions(bool ative){
    if(ative) setMarginBotton(56.0);
    else setMarginBotton(0.0);
    _optionsSC.sink.add(ative);
  }

  /// Método responsável por alterar a quantidade de produtos selecionados,
  /// a partir dele que é habilitado as opções de avançar para próxima página.
  void selecionarProduto(int categoriaIndex, int produtoIndex, int quantidade){

    _listCategorias[categoriaIndex].listProdutos[produtoIndex].selecionado = !_listCategorias[categoriaIndex].listProdutos[produtoIndex].selecionado;
    _listCategorias[categoriaIndex].listProdutos[produtoIndex].quantidade = quantidade;
    ...
    for( var i = 0 ; i < _listCategorias.length; i++ ) {
      for( var j = 0 ; j < _listCategorias[i].listProdutos.length ; j++ ) {
        ...
        if(_listCategorias[i].listProdutos[j].selecionado && _listCategorias[i].listProdutos[j].quantidade > 0) {
          _listProdutosSelecionados.add(_listCategorias[i].listProdutos[j]);
          valorPagar += (_listCategorias[i].listProdutos[j].valor * _listCategorias[i].listProdutos[j].quantidade);
        }
      }
    }
  ...
  /// Método responsável por mudar a página.
  int changePage(int aux){
    ...
    switch(this.page){
      case 1: _progressoSC.add(0.33); break;
      case 2: _progressoSC.add(0.66); break;
      case 3: _progressoSC.add(1); break;
    }
  }
